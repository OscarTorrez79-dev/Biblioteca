<?php
  ...
  $password = $_POST['agregar_password'];
  $password_confirmar = $_POST['confirmar_password_ag'];

  $consulta_usuario = $conn->query("SELECT usuario FROM registros.usuarios WHERE usuario = '$usuario';");
  ...
  if ($password == $password_confirmar) {
    if ($usuario == $user_name) {
      header('Refresh: 3; URL=crear_cuenta.php');
      echo '
      <?php
      session_start();
      ?>
            ...
            <div class="abs-center">
              <form action="editar_usuario.php" onsubmit="return buscar()" method="post" class="border p-3 form">
                <div class="text-center">
                  <h2 class="h2-login">Crear cuenta nueva</h2>
                </div>
                ...
                <div class="alert alert-warning text-center" role="alert">
                  El nombre de usuario ya existe, intenta nuevamente
                </div>
                <div>
      ...
      $query_agregar = $conn->prepare("INSERT INTO registros_biblioteca.usuarios(usuario, contrasenia) VALUES(?,?);");
      $resultado = $query_agregar->execute([$usuario, $hash]);
